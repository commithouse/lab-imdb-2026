# 🧪 LABS PRÁTICOS: Hands-On Exercises (macOS)

Comandos quebrados por linha para rodar no **Terminal** do macOS (bash/zsh). Sem Python.

**Pré-requisitos macOS:** Docker instalado. Para `redis-cli` no host: `brew install redis` (ou use apenas dentro dos containers).
...
Criar diretórios (opcional; no macOS `/tmp` já existe):
...
Sensores 1–50 (no terminal, fora do redis-cli; requer módulo RedisTimeSeries na imagem):
...
Para 1M (demora mais; pode usar um script .sh com o mesmo loop e `seq 1 1000000`).
...
| 3   | RDB/AOF     | `-v /tmp/redis-*`, `for i in {1..100}` |
| 4   | RediSearch  | `FT.CREATE`, `FT.SEARCH`, loop bash para 500 bikes |
| 5   | Write-Behind| Docker Redis + Postgres |
| 6   | Time-Series | `TS.CREATE` / `TS.ADD`, loop bash |
| 10  | Benchmark   | `time` + loop `redis-cli SET` |

**Preparado por:** Prof. Daniel Lemeszenski  
**Data:** 27 Fevereiro 2026  
**Para:** MBA FIAP Encontros 1-3 — versão macOS, sem Python.
